...
class AppiumPCDriver extends BaseDriver {
    ...
    elementCache = {}
    desiredCapConstraints = { listenPort: { presence: true, isString: true } }

    async createSession(...args) {
        const res = await super.createSession(...args)
        // sessionId, caps
        log.info(`listenPort = '${this.opts.listenPort}'`)
        return res
    }
    ...
    async httpGet(api, map_args) {
        ...
        const url = "http://127.0.0.1:" + this.opts.listenPort + "/" + api + args;
    ...
    async getScreenshot() {
        const res = await this.httpGet('getScreenshot')
        return res;
    }

    async getWindowRect() {
        const res = await this.httpGet('getWindowRect')
        const obj = JSON.parse(res);
        return obj;
    }

    async getPageSource() {
        const res = await this.httpGet('getPageSource')
        return res;
    }

    async findElOrEls(strategy, selector, multiple, context) {
        const res = await this.httpGet('findElOrEls', new Map([
            ["strategy", strategy ?? ""],
            ["selector", selector ?? ""],
            ["multiple", multiple ?? ""],
            ["context", context ?? ""]
    ...
    async click(elementId) {
        const res = await this.httpGet('click', new Map([["elementId", elementId ?? ""]]));
    }

    async performTouch(args) {
        args.forEach(async obj => {
            const { action, options: { element, duration, x, y, count } } = obj;
            const res = await this.httpGet('performTouch', new Map([
                ["action", action ?? ""],
                ["element", element ?? ""],
                ["duration", duration ?? ""],
                ["x", x ?? ""],
    ...
    async touchLongClick(elementId) {
        const res = await this.httpGet('touchLongClick', new Map([["elementId", elementId ?? ""]]));
    }

    async getText(elementId) {
        const res = await this.httpGet('getText', new Map([["elementId", elementId ?? ""]]));
        return res;
    }
    ...
    async setValue(text, elementId) {
        ...
        const res = await this.httpGet('setValue', new Map([
            ["text", text ?? ""],
            ["elementId", elementId ?? ""]
        ]));
    }
    ...
    async clear(elementId) {
        const res = await this.httpGet('clear', new Map([["elementId", elementId ?? ""]]));
    }

    async getAttribute(name, elementId) {
        const res = await this.httpGet('getAttribute', new Map([
            ["name", name ?? ""],
            ["elementId", elementId ?? ""]
        ]));
        return res
